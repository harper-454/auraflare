...
const CAPABILITIES: Capability[] = [
  ...
  {
    icon: ImageIcon, label: 'Image', hint: 'SDXL on the edge — type /image',
    prompt: '/image ',
  },
  {
  ...
  const generateImage = async (prompt: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        id: `img-ai-${Date.now()}`,
        role: 'ai',
        content: `![${prompt}](${data.url})\n\n*${prompt}* — generated with SDXL-Lightning on Workers AI, stored in R2.`,
          ...
          >
            ...
            <div className="px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-xs font-mono">
              ...
              <div>
                ...
                <div className="text-slate-300">/image → SDXL-L → R2</div>
              </div>
            </div>
          </motion.div>
        )}
